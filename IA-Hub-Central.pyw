...
import threading, requests, socket, os
...
def check_manus():
    try:
        ...
        s.settimeout(1)
        s.connect(("127.0.0.1", COMET_PORT))
        s.close()
        return True
    except:
        return False
...
class App:
    ...
    def log(s, m, t="i"):
        ...
        s.out.see(tk.END)
    def on_enter(s, e):
        if not e.state & 1: s.send(); return "break"
    def send(s, f=None):
        ...
        r, cp = f, p
        if not r:
            if p.lower().startswith("manus:"): r, cp = "manus", p[6:].strip()
            elif p.lower().startswith("llama:"): r, cp = "llama", p[6:].strip()
            else: r = decide(p)
        def proc():
            s.btn.config(state=tk.DISABLED)
            if r == "manus": s.log(f"Manus: {cp}", "m"); s.log(send_manus(cp), "m")
            else: s.log(f"Llama: {cp}", "l"); s.log(send_ollama(cp), "l")
